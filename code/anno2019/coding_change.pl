...
our $REVISION = '$Revision: 5f52951894c55a0ecb72a2443adab443747dc6b7 $';
...
while (<EVF>) {
	...
	while ($field[2] =~ m/[\w\-\.]+?:([\w\.]+?):exon\d+:c.([\w>]+)(:p.([\w\*]+))?/g) {
		...
		if ($cchange =~ m/^([ACGTacgt])(\d+)([ACGTacgt])$/) {
			...
			} elsif ($cchange =~ m/^([ACGTacgt])(1|2|3)([ACGTacgt])$/) {		#first codon (start codon) is changed, so regardless of -includesnp, this is treated as startloss
				($start, $end, $ref, $obs) = ($2, $2, $1, $3);
			} else {
			...
			} elsif ($cchange =~ m/^(1|2|3)([ACGTacgt])>([ACGTacgt])$/) {		#first codon (start codon) is changed, so regardless of -includesnp, this is treated as startloss
				($start, $end, $ref, $obs) = ($2, $2, $1, $3);
			} else {
				next;
			}
